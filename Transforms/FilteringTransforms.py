...
class FilteringTransforms(FilteringTransforms_Base):
    ...
    def to_cardinal(self, number):
        ...
        if '.' in n:
            ...
            decimal_part = ((ZERO[0] + ' ') * leading_zero_count +
                            self.to_words(int(right)))
            return u'%s %s %s' % (
                self.to_words(int(left)),
                self.pointword,
                decimal_part
            )
        else:
            return self.to_words(int(n))
    ...
    def _money_verbose(self, number, currency):
        return self.to_words(number, currency == 'UAH')

    def _cents_verbose(self, number, currency):
        return self.to_words(number, currency in ('UAH', 'RUB', 'BYN'))

    def to_words(self, n, feminine=False):
        if n < 0:
            return ' '.join([self.negword, self.to_words(abs(n))])
